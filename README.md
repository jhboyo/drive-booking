...
[uv](https://docs.astral.sh/uv/)는 Rust로 작성된 고속 Python 패키지 관리자임. pip 대비 10-100배 빠른 속도를 제공함.
...
```bash
# 기본 학습 (1000 에피소드)
uv run python -m src.train_phase1

# 옵션 지정
uv run python -m src.train_phase1 --episodes 2000 --seed 42 --save-model

# 하이퍼파라미터 조정
uv run python -m src.train_phase1 --lr 0.1 --gamma 0.95 --epsilon-decay 0.998
```
...
# 결과를 JSON 파일로 저장
uv run python -m src.evaluate_phase1 --save-results
```
...
```bash
# 기본 학습 (500 에피소드)
uv run python -m src.train_phase2

# 옵션 지정
uv run python -m src.train_phase2 --episodes 1000 --seed 42
```
...
```bash
# DQN vs 베이스라인 비교 평가
uv run python -m src.evaluate_phase2

# 평가 에피소드 수 조정
uv run python -m src.evaluate_phase2 --episodes 200
```

### Phase 3 학습 (통합 시스템)

```bash
# Phase 3 통합 학습 (사전학습 포함)
uv run python -m src.train_phase3

# 옵션 지정
uv run python -m src.train_phase3 --phase1-episodes 1000 --phase2-episodes 1000 --integrated-episodes 1000
```

| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `--phase1-episodes` | 1000 | Phase 1 사전학습 에피소드 수 |
| `--phase2-episodes` | 1000 | Phase 2 사전학습 에피소드 수 |
| `--integrated-episodes` | 1000 | 통합 학습 에피소드 수 |
| `--seed` | 42 | 랜덤 시드 |

### Phase 3 평가

```bash
# Phase 3 평가 (베이스라인 비교 포함)
uv run python -m src.evaluate_phase3

# 평가 에피소드 수 조정
uv run python -m src.evaluate_phase3 --episodes 200
```
...
```bash
# Phase 1 결과 시각화
uv run python -m src.visualization.plot_phase1_results

# Phase 3 결과 시각화 (학습 곡선, 성능 비교, 시너지 분석)
uv run python -m src.visualization.plot_phase3
```

### 전체 파이프라인 실행 (Quick Start)
...
# 통합 시스템 실행 (Phase 1, 2 사전학습 + 통합 학습 + 평가 + 모델 저장)
uv run python -m src.integrated_system
```

이 명령어 하나로:
- Phase 1 사전학습 (50 에피소드)

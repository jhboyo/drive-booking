...
```
입력 (159차원) → FC(128) + ReLU → FC(128) + ReLU → 출력 (6 액션)
```
...
**학습 조건**: Phase 1 사전학습 300 에피소드 + Phase 2 사전학습 300 에피소드 + 통합 학습 500 에피소드
...
| **Phase 3 (Integrated)** | **23.9** | **83.0%** | **+64.8%** |

#### 보상 구성

| 구성 요소 | 평균 보상 | 비율 |
|-----------|-----------|------|
| Phase 1 보상 | 9.26 | 38.8% |
| Phase 2 보상 | 10.51 | 44.0% |
| Synergy 보너스 | 4.10 | 17.2% |
...
```bash
# Phase 3 통합 학습
python -m src.train_phase3 --episodes 500

# Phase 3 평가
python -m src.evaluate_phase3 --episodes 100 --compare-baselines

# Phase 3 시각화 (학습 + 평가 + 시각화 전체 실행)
python -m src.visualization.plot_phase3
```

| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `--episodes` | 500 | Phase 3 통합 학습 에피소드 수 |
| `--phase1-pretrain` | 300 | Phase 1 사전 학습 에피소드 수 |
| `--phase2-pretrain` | 300 | Phase 2 사전 학습 에피소드 수 |
| `--seed` | 42 | 랜덤 시드 |
| `--save-model` | False | 모델 저장 여부 |
| `--save-results` | False | 결과 JSON 저장 여부 |
| `--compare-baselines` | False | 베이스라인 비교 포함 |
...
### Phase 3 (통합 시스템)
- [x] 전체 예약 성공률 75% 이상 (달성: **83.0%**)
- [x] 총 상호작용 횟수 5회 이하 (달성: **2.57회**)
- [x] 개별 Phase 대비 Synergy Bonus 10% 이상 (달성: **17.2%**)
- [x] Random 베이스라인 대비 20% 이상 성능 향상 (달성: **+64.8%**)
